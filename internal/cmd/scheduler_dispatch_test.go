...
// captureCombinedOutput redirects both os.Stdout and os.Stderr into one pipe,
// mirroring exec.Cmd.CombinedOutput — which is how the witness's SLOT_OPEN
// trigger actually reads `gt scheduler status --json`.
...
// TestRunSchedulerStatusJSONStaysParseableWithSkippedContexts pins the contract
// the SLOT_OPEN trigger depends on. The witness reads this command with
// CombinedOutput, so the skipped-context warnings land in the same buffer as
// the JSON; it recovers by trimming everything before the first "{". This test
// reproduces that exact read, so a reporting change that reintroduces the
// town-wide dispatch outage fails here instead of in production.
func TestRunSchedulerStatusJSONStaysParseableWithSkippedContexts(t *testing.T) {
	townRoot := setupSchedulerPartialScanTown(t, "gt-work")
	oldCWD, err := os.Getwd()
	if err != nil {
		t.Fatalf("getwd: %v", err)
	...
	t.Cleanup(func() { _ = os.Chdir(oldCWD) })
	oldJSON := schedulerStatusJSON
	schedulerStatusJSON = true
	t.Cleanup(func() { schedulerStatusJSON = oldJSON })

	var statusErr error
	combined := captureCombinedOutput(t, func() { statusErr = runSchedulerStatus(nil, nil) })
	if statusErr != nil {
		t.Fatalf("scheduler status --json failed on one unresolvable context: %v", statusErr)
	}
	if !strings.Contains(combined, filepath.Join("rig", ".beads")) {
		t.Fatalf("combined output = %q, want the skipped context named", combined)
	}

	// Exactly the witness's recovery in readSchedulerStatusForSlotOpen.
	jsonOutput := strings.TrimSpace(combined)
	...
	var status struct {
		...
		Skipped []skippedContextInfo `json:"skipped_contexts"`
	}
	if err := json.Unmarshal([]byte(jsonOutput), &status); err != nil {
		t.Fatalf("SLOT_OPEN parse of scheduler status failed: %v\ncombined: %q", err, combined)
	}
	if status.QueuedReady != 1 {
		t.Errorf("queued_ready = %d, want the resolvable context still counted as ready", status.QueuedReady)
	}
	if len(status.Skipped) != 1 || !strings.Contains(status.Skipped[0].BeadsDir, filepath.Join("rig", ".beads")) {
		t.Errorf("skipped_contexts = %+v, want the unresolvable context reported in JSON", status.Skipped)
	}
}
